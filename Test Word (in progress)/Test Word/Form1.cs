...
namespace Test_Word
{
    public partial class Form1 : Form
    {
        ...
        {
            CreateSetupAct CSA = new CreateSetupAct();
          /*  Word.Application app = new Word.Application
            {
                Visible = false
            };*/
            CSA.BTNCreateSetupAct(sender, e);
        ...
        private void BTNCreateTemplate1_Click(object sender, EventArgs e)
        {
            if (CBFoundationContracter.Checked)
            {
                ...
                {
                    var doc = app.Documents.Open(Application.StartupPath + @"\ActSetup.dot");
                    if (TBDirectorPost1.Text != "") { ReplaceWordStub("$DirectorPost", TBDirectorPost1.Text, doc);}
                    if (TBDirectorName1.Text != "") { ReplaceWordStub("$DirectorIni", ToIniRollover(TBDirectorName1.Text), doc); }
                    if (TBFoundation1.Text != "") { ReplaceWordStub("$FoundationClient", ToIniRollover(TBFoundation1.Text), doc); }
                    if (TBFoundation1.Text != "") { ReplaceWordStub("$FoundationClient", ToIniRollover(TBFoundation1.Text), doc); }
                    if (CBUnit1.SelectedItem.ToString() != "") { ReplaceWordStub("$FoundationClient", CBUnit1.SelectedItem.ToString(), doc); }
                    if (CBUnit1.SelectedItem.ToString() != "") { ReplaceWordStub("$FoundationClient", CBUnit1.SelectedItem.ToString(), doc); }
                    ReplaceWordStub("$FoundationContracter", "Хабаровского НТЦ «ФГУП НПП «Гамма»", doc);
                    ReplaceWordStub("$FoundationContracter", "Хабаровского НТЦ «ФГУП НПП «Гамма»", doc);
                    ReplaceWordStub("$FoundationContracter", "Хабаровского НТЦ «ФГУП НПП «Гамма»", doc);
                    if (TBFoundationVP1.Text != "") { ReplaceWordStub("$FoundVP", TBFoundationVP1.Text, doc); }
                    if (TBDay1.Text != "") { ReplaceWordStub("$Day", TBDay1.Text, doc); }
                    if (CBMonth1.SelectedItem.ToString() != "") { ReplaceWordStub("$Month", CBMonth1.SelectedItem.ToString(), doc); }
                    if (TBYear1.Text != "") { ReplaceWordStub("$Year", TBYear1.Text, doc); }
                    if (TBYear1.Text != "") { ReplaceWordStub("$Year", TBYear1.Text, doc); }
                    if (TBYear1.Text != "") { ReplaceWordStub("$Year", TBYear1.Text, doc); }
                    if (TBUser1.Text != "") { ReplaceWordStub("$UserIni", TBUser1.Text, doc); }
                    if (TBUser1.Text != "") { ReplaceWordStub("$UserIni", TBUser1.Text, doc); }
                    if (TBUser1.Text != "") { ReplaceWordStub("$UserIni", TBUser1.Text, doc); }
                    if (TBWorkerPost1.Text != "") { ReplaceWordStub("$WorkerPost", TBWorkerPost1.Text, doc); }
                    if (TBWorkerPost1.Text != "") { ReplaceWordStub("$WorkerPost", TBWorkerPost1.Text, doc); }
                    if (TBWorker1.Text != "") { ReplaceWordStub("$WorkerName", TBWorker1.Text, doc); }
                    if (TBWorker1.Text != "") { ReplaceWordStub("$WorkerIni", ToIni(TBWorker1.Text), doc); }
                    if (TBPostRes1.Text != "") { ReplaceWordStub("$AdminPost", TBPostRes1.Text, doc); }
                    if (TBPostRes1.Text != "") { ReplaceWordStub("$AdminPost", TBPostRes1.Text, doc); }
                    if (TBPostRes1.Text != "") { ReplaceWordStub("$AdminPost", TBPostRes1.Text, doc); }
                    if (TBResponsible1.Text != "") { ReplaceWordStub("$AdminName", TBResponsible1.Text, doc); }
                    if (TBResponsible1.Text != "") { ReplaceWordStub("$AdminIni", ToIni(TBResponsible1.Text), doc); }
                    if (TBCabinet1.Text != "") { ReplaceWordStub("$CabinetNum", TBCabinet1.Text, doc); }
                    if (TBPC1.Text != "") { ReplaceWordStub("$PCNumber", TBPC1.Text, doc); }
                    if (TBOrder1.Text != "") { ReplaceWordStub("$Order", TBOrder1.Text, doc); }
                    if (TBOrder11.Text != "") { ReplaceWordStub("$Print", TBOrder11.Text, doc); }
                    if (TBOrder12.Text != "") { ReplaceWordStub("$Print", TBOrder12.Text, doc); }
                    if (CBSKZI1.SelectedItem.ToString() != "") { ReplaceWordStub("$VersionSKZI", CBSKZI1.SelectedItem.ToString(), doc); }
                    if (CBSKZI1.SelectedItem.ToString() != "") { ReplaceWordStub("$VersionSKZI", CBSKZI1.SelectedItem.ToString(), doc); }
                    string m = "";
                    switch (CBAct1.SelectedIndex)
                    {
                    ...
                    if (TBRegCard1.Text != "") { ReplaceWordStub("$UserName", TBRegCard1.Text, doc); }
                    if (TBKey1.Text != "") { ReplaceWordStub("$DSTNum", TBKey1.Text, doc); }
                    if (TBRegCard1.Text != "") { ReplaceWordStub("$RegisterCardNum", TBRegCard1.Text, doc); }
                    ReplaceWordStub("$Instruction", "Да", doc);
                    ReplaceWordStub("$Contacts", "Да", doc);
                }
                catch (Exception ex)
